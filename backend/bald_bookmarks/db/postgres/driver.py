"""PostgreSQL DatabaseDriver implementation using psycopg."""
...
class PostgresDriver(DatabaseDriver):
    """Persist Bald Bookmarks data with a psycopg connection pool."""

    def __init__(self, settings: Settings) -> None:
        """Store settings for pool creation.

        Args:
            settings (Settings): Application settings with PostgreSQL credentials.
        """
        self._settings = settings
        self._pool: ConnectionPool | None = None

    def connect(self) -> None:
        """Create the psycopg connection pool.
        ...
        if self._pool is not None:
            return
        args = self._settings.postgres_connect_args()
    ...
    def health_check(self) -> DatabaseHealth:
        ...
        if self._pool is None:
            return DatabaseHealth(
                ...
                driver="postgres",
                latency_ms=None,
                message="PostgreSQL connection pool is not open",
            )
        try:
            with self._acquire() as conn, conn.cursor() as cursor:
                try:
                    ...
                    logger.bind(host=self._settings.postgres_host).error(
    ...
    def alembic_revision(self) -> str | None:
        ...
        Raises:
            psycopg.Error: If alembic_version cannot be queried.
    ...
    def _acquire(self) -> psycopg.Connection:
        ...
        Returns:
            psycopg.Connection: Open connection.

        Raises:
            RuntimeError: If the pool has not been created.
        """
        if self._pool is None:
            raise RuntimeError("PostgresDriver is not connected")
    ...
    def _insert_id(
        ...
        Args:
            cursor (psycopg.Cursor[Any]): Active cursor.
    ...
    def _fetch_folder(self, conn: psycopg.Connection, folder_id: int) -> Folder:
        """Load a folder row.

        Args:
            conn (psycopg.Connection): Active connection.
    ...
    def _bookmark_counts(self, conn: psycopg.Connection) -> dict[int, int]:
    ...
    def _bookmark_tags(self, conn: psycopg.Connection, bookmark_id: int) -> list[Tag]:
    ...
    def _fetch_bookmark(self, conn: psycopg.Connection, bookmark_id: int) -> Bookmark:
    ...
    def _get_or_create_tag(self, conn: psycopg.Connection, name: str) -> Tag:
