"""MySQL DatabaseDriver implementation using mysql-connector-python."""
...
WITH RECURSIVE descendants AS (
    SELECT id FROM folders WHERE parent_id = %(id)s
    UNION ALL
    SELECT f.id
    FROM folders f
...
WITH RECURSIVE tree AS (
    SELECT id, 0 AS lvl FROM folders WHERE id = %(id)s
    UNION ALL
...
class MySQLDriver(DatabaseDriver):
    """Persist Bald Bookmarks data with a mysql-connector connection pool."""

    def __init__(self, settings: Settings) -> None:
        """Store settings for pool creation.

        Args:
            settings (Settings): Application settings with MySQL credentials.
        """
        self._settings = settings
        self._pool: MySQLConnectionPool | None = None

    def connect(self) -> None:
        """Create the mysql-connector connection pool.
        ...
        if self._pool is not None:
            return
        args = self._settings.mysql_connect_args()
        self._pool = MySQLConnectionPool(
            pool_name="bald_bookmarks",
            pool_size=4,
            host=str(args["host"]),
            port=int(args["port"]),
            user=str(args["user"]),
    ...
    def health_check(self) -> DatabaseHealth:
        ...
        if self._pool is None:
            return DatabaseHealth(
                ...
                driver="mysql",
                latency_ms=None,
                message="MySQL connection pool is not open",
            )
        try:
            with self._acquire() as conn, conn.cursor() as cursor:
                try:
                    ...
                    logger.bind(host=self._settings.mysql_host).error(
    ...
    def alembic_revision(self) -> str | None:
        ...
        Raises:
            mysql.connector.Error: If alembic_version cannot be queried.
    ...
    @contextmanager
    def _acquire(self) -> Iterator[PooledMySQLConnection]:
        """Acquire a pooled connection and return it when done.

        Yields:
            PooledMySQLConnection: Open connection from the pool.
    ...
    def _insert_id(
        ...
        Args:
            cursor (Any): Active MySQL cursor.
            sql (str): INSERT statement.
            params (dict[str, Any]): Named bind values.

        Returns:
            int: Generated primary key from lastrowid.
        """
        cursor.execute(sql, params)
        return int(cursor.lastrowid)

    def _fetch_folder(self, conn: PooledMySQLConnection, folder_id: int) -> Folder:
        """Load a folder row.

        Args:
            conn (PooledMySQLConnection): Active connection.
    ...
    def _bookmark_counts(self, conn: PooledMySQLConnection) -> dict[int, int]:
    ...
    def _bookmark_tags(
        self, conn: PooledMySQLConnection, bookmark_id: int
    ) -> list[Tag]:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                WHERE bt.bookmark_id = %(bookmark_id)s
                ORDER BY t.name
                """,
    ...
    def _get_or_create_tag(self, conn: PooledMySQLConnection, name: str) -> Tag:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                "SELECT id, name, created_at FROM tags WHERE name = %(name)s",
                {"name": normalized},
            )
            row = cursor.fetchone()
            if row is not None:
    ...
    def _set_bookmark_tags(
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                "DELETE FROM bookmark_tags WHERE bookmark_id = %(bookmark_id)s",
                {"bookmark_id": bookmark_id},
            )
        ...
        with self._acquire() as conn:
            ...
            with conn.cursor() as cursor:
                folder_id = self._insert_id(
                    ...
                    VALUES (
                        %(parent_id)s, %(name)s, %(sort_order)s,
    ...
    def delete_folder(self, folder_id: int, *, recursive: bool = False) -> None:
        ...
        with self._acquire() as conn:
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    "SELECT COUNT(*) FROM folders WHERE parent_id = %(id)s",
                    {"id": folder_id},
                )
                child_count = cursor.fetchone()[0]
                cursor.execute(
                    "SELECT COUNT(*) FROM bookmarks WHERE folder_id = %(id)s",
                    {"id": folder_id},
                )
                ...
                for target_id in targets:
                    cursor.execute(
                        "SELECT id FROM bookmarks WHERE folder_id = %(id)s",
                        {"id": target_id},
                    )
                    bookmark_ids = [int(row[0]) for row in cursor.fetchall()]
                    for bookmark_id in bookmark_ids:
                        cursor.execute(
                            "DELETE FROM bookmark_tags WHERE bookmark_id = %(id)s",
                            {"id": bookmark_id},
                        )
                        cursor.execute(
                            "DELETE FROM bookmarks WHERE id = %(id)s",
                            {"id": bookmark_id},
                        )
                try:
                ...
                for row in ordered:
                    cursor.execute(
                        "DELETE FROM folders WHERE id = %(id)s",
                        {"id": int(row[0])},
                    )
            conn.commit()
        ...
        with self._acquire() as conn:
            ...
            with conn.cursor() as cursor:
                bookmark_id = self._insert_id(
                    ...
                    VALUES (
                        %(folder_id)s, %(title)s, %(url)s, %(description)s,
                        %(thumbnail_status)s, %(created_at)s, %(updated_at)s
                    )
                    """,
                    {
        ...
        *,
        ...
        with self._acquire() as conn, conn.cursor() as cursor:
            ...
            if unfiled_only:
                ...
                clauses.append("b.folder_id = %(folder_id)s")
                params["folder_id"] = folder_id
            if q:
                clauses.append(
                    "("
                    "LOWER(b.title) LIKE %(q)s OR LOWER(b.url) LIKE %(q)s "
                    "OR LOWER(COALESCE(b.description, '')) LIKE %(q)s"
                    ")"
                )
                params["q"] = f"%{q.lower()}%"
            if tag:
                clauses.append(
                    ...
                    "WHERE bt.bookmark_id = b.id AND t.name = %(tag_name)s"
                    ")"
                )
        ...
        with self._acquire() as conn:
            ...
            with conn.cursor() as cursor:
                cursor.execute(
                    """
                    UPDATE bookmarks
                    SET thumbnail_status = %(thumbnail_status)s,
                        thumbnail_path = %(thumbnail_path)s,
                        thumbnail_updated_at = %(thumbnail_updated_at)s,
                        updated_at = %(updated_at)s
                    WHERE id = %(id)s
                    """,
                    {
    ...
    def create_tag(self, payload: TagCreate) -> Tag:
        ...
        with self._acquire() as conn, conn.cursor() as cursor:
            cursor.execute(
                "SELECT id FROM tags WHERE name = %(name)s",
                {"name": payload.name},
            )
            ...
            conn.commit()
            cursor.execute(
                "SELECT id, name, created_at FROM tags WHERE id = %(id)s",
                {"id": tag_id},
            )
            row = cursor.fetchone()
            return Tag(id=int(row[0]), name=row[1], created_at=row[2])
    ...
    def delete_tag(self, tag_id: int) -> None:
        ...
        with self._acquire() as conn, conn.cursor() as cursor:
            ...
            cursor.execute(
                "DELETE FROM bookmark_tags WHERE tag_id = %(id)s",
                {"id": tag_id},
            )
            cursor.execute("DELETE FROM tags WHERE id = %(id)s", {"id": tag_id})
            conn.commit()
    ...
    def enqueue_job(self, payload: JobEnqueue) -> Job:
        ...
        with self._acquire() as conn, conn.cursor() as cursor:
            ...
            job_id = self._insert_id(
                ...
                VALUES (
                    %(job_type)s, %(payload_json)s, %(status)s, 0, %(max_attempts)s,
                    %(scheduled_at)s, %(created_at)s, %(updated_at)s
                )
                """,
                {
    ...
    def claim_next_job(self) -> Job | None:
        ...
        with self._acquire() as conn, conn.cursor() as cursor:
            try:
                cursor.execute(
                    _CLAIM_NEXT_JOB_SQL,
                    {"pending": JobStatus.PENDING.value, "now": now},
                )
                row = cursor.fetchone()
                if row is None:
                    conn.rollback()
